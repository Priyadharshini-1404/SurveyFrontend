...
export default function RequestSurvey() {
  ...
  useFocusEffect(useCallback(() => {
    if (route.params?.selectedLocation) setLocation(route.params.selectedLocation);
  }, [route.params]));

  const handleDateChange = (event, selectedDate) => {
    setShowDatePicker(false);
    if (selectedDate) setSurveyDate(selectedDate.toISOString().split("T")[0]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        name, surveyType, location, contact, surveyDate,
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.dropdownButton} onPress={() => setModalVisible(true)}>
            <Text style={[styles.dropdownText, { color: surveyType ? "#000" : "#888" }]}>{surveyType || "Select Survey Type"}</Text>
            <Ionicons name="chevron-down" size={20} color="#777" />
          </TouchableOpacity>

          <Text style={styles.label}>Location</Text>
          <View style={styles.locationContainer}>
            <TextInput style={[styles.input, { flex: 1 }]} placeholder="Choose a location" value={location} editable={false} />
            <TouchableOpacity onPress={() => navigation.navigate("MapPickerScreen", { from: "RequestSurvey", formState: { name, surveyType, contact, surveyDate } })}>
          ...
          <TouchableOpacity style={styles.input} onPress={() => setShowDatePicker(true)}>
            <Text style={{ fontSize: 16, color: surveyDate ? "#000" : "#888" }}>{surveyDate || "Select date"}</Text>
          </TouchableOpacity>

          {showDatePicker && (<DateTimePicker value={surveyDate ? new Date(surveyDate) : new Date()} mode="date" display="calendar" onChange={handleDateChange} />)}

          <Text style={styles.label}>Contact Number</Text>
          <TextInput style={styles.input} placeholder="Enter mobile number" keyboardType="phone-pad" value={contact} onChangeText={setContact} />

          <TouchableOpacity style={[styles.button, { opacity: loading ? 0.7 : 1 }]} disabled={loading} onPress={handleSubmit}>
...
const styles = StyleSheet.create({
  ...
  mainHeader: { fontSize: 26, fontWeight: "bold", color: "#0a67d0", textAlign: "center", marginBottom: 20 },
  card: { backgroundColor: "#fff", padding: 20, borderRadius: 16, shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 6, elevation: 3 },
  label: { fontSize: 15, fontWeight: "600", marginTop: 12, marginBottom: 6, color: "#222" },
  input: { backgroundColor: "#f1f4f9", padding: 14, borderRadius: 10, fontSize: 16 },
  dropdownButton: { backgroundColor: "#f1f4f9", padding: 14, borderRadius: 10, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  dropdownText: { fontSize: 16 },
  locationContainer: { flexDirection: "row", alignItems: "center", gap: 10 },
  button: { backgroundColor: "#0a67d0", paddingVertical: 16, borderRadius: 12, marginTop: 25 },
  ...
  modalContent: { backgroundColor: "#fff", borderRadius: 14, padding: 20, maxHeight: "70%" },
  modalTitle: { fontSize: 20, fontWeight: "bold", marginBottom: 10, textAlign: "center" },
  modalItem: { paddingVertical: 12 },
  modalItemText: { fontSize: 16, textAlign: "center" },
  modalCloseButton: { backgroundColor: "#ff3b30", padding: 10, borderRadius: 10, marginTop: 12 },
